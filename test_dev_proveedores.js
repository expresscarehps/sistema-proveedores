...
console.log('\n=== Codificación de encabezados de correo (evita símbolos raros con acentos o guiones largos) ===');
...
assert(codificarEncabezadoUTF8('Registro aprobado - Portal de Proveedores') === 'Registro aprobado - Portal de Proveedores',
  'Un asunto en puro ASCII (sin acentos ni símbolos especiales) se queda tal cual, sin codificar de más');
const asuntoConGuionLargo = 'Contrarecibo generado — factura 4694';
const asuntoCodificado = codificarEncabezadoUTF8(asuntoConGuionLargo);
assert(asuntoCodificado.startsWith('=?UTF-8?B?') && asuntoCodificado.endsWith('?='),
  'Un asunto con guión largo (—) se codifica en formato RFC 2047, para que no salga con símbolos raros');
assert(Buffer.from(asuntoCodificado.replace('=?UTF-8?B?', '').replace('?=', ''), 'base64').toString('utf-8') === asuntoConGuionLargo,
  'Al decodificar de vuelta, se recupera exactamente el texto original con el guión largo intacto');
const asuntoConAcentos = 'Envío de información — próxima validación';
const acentosCodificado = codificarEncabezadoUTF8(asuntoConAcentos);
assert(Buffer.from(acentosCodificado.replace('=?UTF-8?B?', '').replace('?=', ''), 'base64').toString('utf-8') === asuntoConAcentos,
  'También funciona correctamente con acentos normales (í, ó), no solo con el guión largo');
