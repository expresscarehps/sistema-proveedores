...
function filtrarYOrdenarContrarecibos(filas) {
  // Se incluye cualquier factura que YA tenga folio de contrarecibo, sin importar
  ...
  const conContrarecibo = filas.filter(f => !!f.folio);
  return conContrarecibo.reverse();
}
const filasPrueba = [
  { folio: 'CR-KARALV-2026-001', estatus: 'Validada' },
  { folio: '', estatus: 'Pendiente de validar' },
  { folio: 'CR-KARALV-2026-002', estatus: 'Pagada' },
  { folio: '', estatus: 'Rechazada' }
];
const resultadoFiltrado = filtrarYOrdenarContrarecibos(filasPrueba);
assert(resultadoFiltrado.length === 2, 'Solo incluye facturas que YA tienen folio de contrarecibo, ignora Pendientes y Rechazadas (que nunca tuvieron uno)');
assert(resultadoFiltrado[0].folio === 'CR-KARALV-2026-002', 'El más reciente aparece primero en la lista');
assert(resultadoFiltrado.some(f => f.estatus === 'Pagada'), 'Una factura YA PAGADA sigue apareciendo en Contrarecibos — no debe desaparecer solo porque ya se pagó');
