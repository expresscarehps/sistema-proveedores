...
const VERSION_CODIGO = 'v14-fase2-registro-facturas'; // Cambia en cada entrega, para confirmar que el código desplegado es el más reciente
...
function registrarFactura(data) {
  ...
  const alertaRFC = datosXML.rfcEmisor.toUpperCase() !== proveedor.rfc.toUpperCase()
    ? 'El RFC del XML (' + datosXML.rfcEmisor + ') no coincide con el registrado (' + proveedor.rfc + ')'
    : '';
  ...
  hoja.appendRow([
    ...
    datosXML.rfcEmisor, alertaRFC, datosXML.uuid, datosXML.total, datosXML.fecha,
